...
awk '{if ($7 == "+") print $0}' ${WRKDIR}/src/${STRAIN}/exonsFULL.bed | bedtools intersect -split -f 1.0 -b stdin -abam ${SAMP}.83_163.sorted.bam > ${SAMP}_exons.83_163.tmptemplate.bam
awk '{if ($7 == "-") print $0}' ${WRKDIR}/src/${STRAIN}/exonsFULL.bed | bedtools intersect -split -f 1.0 -b stdin -abam ${SAMP}.83_163.sorted.bam > ${SAMP}_exons.83_163.tmpnontemplate.bam
awk '{if ($7 == "+") print $0}' ${WRKDIR}/src/${STRAIN}/exonsFULL.bed | bedtools intersect -split -f 1.0 -b stdin -abam ${SAMP}.99_147.sorted.bam > ${SAMP}_exons.99_147.tmpnontemplate.bam
awk '{if ($7 == "-") print $0}' ${WRKDIR}/src/${STRAIN}/exonsFULL.bed | bedtools intersect -split -f 1.0 -b stdin -abam ${SAMP}.99_147.sorted.bam > ${SAMP}_exons.99_147.tmptemplate.bam
...
java -Xmx24G -jar ${PICARD}/picard.jar FilterSamReads I=${SAMP}_exons.83_163.tmptemplate.bam FILTER=includeReadList RLF=${SAMP}_exons.83_163.tmptemplate.tag_names.txt O=${SAMP}_exons.83_163.template.bam MAX_RECORDS_IN_RAM=50000
java -Xmx24G -jar ${PICARD}/picard.jar FilterSamReads I=${SAMP}_exons.83_163.tmpnontemplate.bam FILTER=includeReadList RLF=${SAMP}_exons.83_163.tmpnontemplate.tag_names.txt O=${SAMP}_exons.83_163.nontemplate.bam MAX_RECORDS_IN_RAM=50000
java -Xmx24G -jar ${PICARD}/picard.jar FilterSamReads I=${SAMP}_exons.99_147.tmpnontemplate.bam FILTER=includeReadList RLF=${SAMP}_exons.99_147.tmpnontemplate.tag_names.txt O=${SAMP}_exons.99_147.nontemplate.bam MAX_RECORDS_IN_RAM=50000
java -Xmx24G -jar ${PICARD}/picard.jar FilterSamReads I=${SAMP}_exons.99_147.tmptemplate.bam FILTER=includeReadList RLF=${SAMP}_exons.99_147.tmptemplate.tag_names.txt O=${SAMP}_exons.99_147.template.bam MAX_RECORDS_IN_RAM=50000
...
samtools merge -sorted ${SAMP}_all_exons.sorted.bam ${SAMP}_exons_template.sorted.bam ${SAMP}_exons_nontemplate.sorted.bam 
samtools view ${SAMP}_all_exons.sorted.bam | awk '{print $1}' > ${SAMP}_all_exons.tag_names.txt

#start getting intronic
awk '{ if (($9 == "protein_coding") && ($10 =="gene") && ($6 == "+")) print $1"\t"$2"\t"$3"\t"$7"\t"$6}' ${WRKDIR}/src/${STRAIN}/${STRAIN}.gtf.sorted.tsv  | bedtools intersect -b stdin -abam ${SAMP}.83_163.sorted.bam > ${SAMP}_introns.83_163.tmptemplate.bam
awk '{ if (($9 == "protein_coding") && ($10 =="gene") && ($6 == "-")) print $1"\t"$2"\t"$3"\t"$7"\t"$6}' ${WRKDIR}/src/${STRAIN}/${STRAIN}.gtf.sorted.tsv  | bedtools intersect -b stdin -abam ${SAMP}.83_163.sorted.bam > ${SAMP}_introns.83_163.tmpnontemplate.bam
awk '{ if (($9 == "protein_coding") && ($10 =="gene") && ($6 == "+")) print $1"\t"$2"\t"$3"\t"$7"\t"$6}' ${WRKDIR}/src/${STRAIN}/${STRAIN}.gtf.sorted.tsv  | bedtools intersect -b stdin -abam ${SAMP}.99_147.sorted.bam > ${SAMP}_introns.99_147.tmpnontemplate.bam
awk '{ if (($9 == "protein_coding") && ($10 =="gene") && ($6 == "-")) print $1"\t"$2"\t"$3"\t"$7"\t"$6}' ${WRKDIR}/src/${STRAIN}/${STRAIN}.gtf.sorted.tsv  | bedtools intersect -b stdin -abam ${SAMP}.99_147.sorted.bam > ${SAMP}_introns.99_147.tmptemplate.bam
...
rm ${SAMP}_exons.83_163.tmpnontemplate.bam
rm ${SAMP}_exons.83_163.tmpnontemplate.tag_names.txt
rm ${SAMP}_exons.83_163.tmptemplate.bam
rm ${SAMP}_exons.83_163.tmptemplate.tag_names.txt
rm ${SAMP}_exons.99_147.tmpnontemplate.bam
rm ${SAMP}_exons.99_147.tmpnontemplate.tag_names.txt
rm ${SAMP}_exons.99_147.tmptemplate.bam
rm ${SAMP}_exons.99_147.tmptemplate.tag_names.txt
rm ${SAMP}_introns.83_163.tmp2nontemplate.bam
rm ${SAMP}_introns.83_163.tmp2nontemplate.tag_names.txt
rm ${SAMP}_introns.83_163.tmp2template.bam
rm ${SAMP}_introns.83_163.tmp2template.tag_names.txt
rm ${SAMP}_introns.99_147.tmp2nontemplate.bam
rm ${SAMP}_introns.99_147.tmp2nontemplate.tag_names.txt
rm ${SAMP}_introns.99_147.tmp2template.bam
rm ${SAMP}_introns.99_147.tmp2template.tag_names.txt
